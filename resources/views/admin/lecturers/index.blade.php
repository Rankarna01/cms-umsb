@extends('layouts.admin')
@section('title', 'Direktori Dosen')

@section('content')
<link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" rel="stylesheet"/>

<div class="space-y-6">

  {{-- Header --}}
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
      <h1 class="text-3xl font-bold text-slate-800">Direktori Dosen</h1>
      <p class="text-sm text-slate-500">Kelola data dosen per program studi dan fakultas.</p>
    </div>

    <div class="flex items-center gap-3">
      <div class="relative">
        <input id="lectSearch" type="text" placeholder="Cari nama, NIDN, prodi, fakultas…"
               class="peer w-64 sm:w-80 rounded-xl border border-slate-200 bg-white/80 px-10 py-2.5 text-sm text-slate-700 shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none"
               oninput="filterLecturerTable()" />
        <i class="fa-solid fa-magnifying-glass text-slate-400 absolute left-3 top-1/2 -translate-y-1/2"></i>
      </div>

      <a href="{{ route('admin.lecturers.create') }}"
         class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md hover:bg-blue-700 hover:shadow-lg focus:outline-none focus:ring-4 focus:ring-blue-200">
        <i class="fa-solid fa-user-plus"></i>
        Tambah Dosen
      </a>
    </div>
  </div>

  {{-- Alert sukses --}}
  @if(session('success'))
    <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-800 shadow-sm">
      <i class="fa-regular fa-circle-check mr-2"></i>
      <span>{{ session('success') }}</span>
    </div>
  @endif

  {{-- Table --}}
  <div class="overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200 shadow-sm">
    <div class="overflow-x-auto">
      <table id="lectTable" class="min-w-full table-auto">
        <thead class="bg-slate-50 sticky top-0 z-10">
          <tr>
            <th class="px-5 py-3 text-left text-[11px] font-semibold tracking-wider text-slate-600 uppercase">Nama</th>
            <th class="px-5 py-3 text-left text-[11px] font-semibold tracking-wider text-slate-600 uppercase">Jabatan</th>
            <th class="px-5 py-3 text-left text-[11px] font-semibold tracking-wider text-slate-600 uppercase">Program Studi</th>
            <th class="px-5 py-3 text-center text-[11px] font-semibold tracking-wider text-slate-600 uppercase">Aksi</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-100">
          @forelse ($lecturers as $lecturer)
            <tr class="hover:bg-slate-50/60 transition">
              {{-- Nama + NIDN + Avatar --}}
              <td class="px-5 py-4">
                <div class="flex items-center gap-3">
                  <div class="relative">
                    <img class="h-12 w-12 rounded-full object-cover ring-2 ring-white shadow"
                      src="{{ $lecturer->photo ? Storage::url($lecturer->photo) : 'https://ui-avatars.com/api/?name=' . urlencode($lecturer->name) . '&background=random' }}"
                      alt="{{ $lecturer->name }}" />
                    <span class="absolute -bottom-1 -right-1 grid h-5 w-5 place-items-center rounded-full bg-emerald-500 text-white text-[10px] ring-2 ring-white">
                      <i class="fa-solid fa-user-graduate"></i>
                    </span>
                  </div>
                  <div>
                    <p class="font-semibold text-slate-800">{{ $lecturer->name }}</p>
                    <p class="text-xs text-slate-500">{{ $lecturer->nidn }}</p>
                  </div>
                </div>
              </td>

              {{-- Jabatan --}}
              <td class="px-5 py-4">
                <span class="inline-flex items-center gap-2 rounded-lg bg-indigo-50 px-2.5 py-1.5 text-xs font-medium text-indigo-700">
                  <i class="fa-solid fa-briefcase text-indigo-500"></i>
                  {{ $lecturer->position ?? '-' }}
                </span>
              </td>

              {{-- Prodi & Fakultas --}}
              <td class="px-5 py-4">
                <div class="flex flex-col gap-1">
                  <span class="inline-flex items-center gap-2 rounded-lg bg-fuchsia-50 px-2.5 py-1 text-xs font-medium text-fuchsia-700">
                    <i class="fa-solid fa-graduation-cap text-fuchsia-500"></i>
                    {{ $lecturer->studyProgram->name ?? '-' }}
                  </span>
                  @if($lecturer->faculty?->name)
                    <span class="inline-flex items-center gap-2 rounded-lg bg-blue-50 px-2.5 py-1 text-[11px] font-medium text-blue-700">
                      <i class="fa-solid fa-school text-blue-500"></i>
                      {{ $lecturer->faculty->name }}
                    </span>
                  @endif
                </div>
              </td>

              {{-- Aksi --}}
              <td class="px-5 py-4">
                <div class="flex items-center justify-center gap-2">
                  <a href="{{ route('admin.lecturers.edit', $lecturer->id) }}"
                     class="inline-flex items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100 hover:shadow-sm focus:outline-none focus:ring-4 focus:ring-indigo-100"
                     title="Edit">
                    <i class="fa-regular fa-pen-to-square"></i> Edit
                  </a>

                  <form action="{{ route('admin.lecturers.destroy', $lecturer->id) }}" method="POST"
                        class="inline-block"
                        onsubmit="return confirm('Yakin ingin menghapus data dosen ini?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                      class="inline-flex items-center gap-2 rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-100 hover:shadow-sm focus:outline-none focus:ring-4 focus:ring-rose-100"
                      title="Hapus">
                      <i class="fa-regular fa-trash-can"></i> Hapus
                    </button>
                  </form>
                </div>
              </td>
            </tr>
          @empty
            <tr>
              <td colspan="4" class="py-16">
                <div class="mx-auto w-full max-w-md rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center">
                  <div class="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-xl bg-indigo-100 text-indigo-600">
                    <i class="fa-solid fa-user-tie"></i>
                  </div>
                  <p class="font-semibold text-slate-700">Belum ada data dosen</p>
                  <p class="text-sm text-slate-500">Tambahkan data pertama Anda untuk mulai mengelola.</p>
                  <a href="{{ route('admin.lecturers.create') }}"
                     class="mt-4 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md hover:bg-blue-700">
                    <i class="fa-solid fa-user-plus"></i> Tambah Dosen
                  </a>
                </div>
              </td>
            </tr>
          @endforelse
        </tbody>
      </table>
    </div>
  </div>
</div>

<script>
  // Filter client-side (tanpa ubah backend)
  function filterLecturerTable(){
    const q = (document.getElementById('lectSearch').value || '').toLowerCase();
    const rows = document.querySelectorAll('#lectTable tbody tr');
    rows.forEach(row=>{
      const text = row.innerText.toLowerCase();
      row.style.display = text.includes(q) ? '' : 'none';
    });
  }
</script>
@endsection
