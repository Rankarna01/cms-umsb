...
{{-- (opsional) Alpine untuk tooltip sederhana; aman jika sudah ada di layout --}}
...
<div class="space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
      <h1 class="text-3xl font-bold text-slate-800">Manajemen Fakultas</h1>
      <p class="text-sm text-slate-500">Kelola daftar fakultas yang tampil di situs.</p>
    </div>

    <div class="flex items-center gap-3">
      <div class="relative">
        <input id="tableSearch" type="text" placeholder="Cari fakultas…"
  ...
  <div class="overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200 shadow-sm">
    <div class="overflow-x-auto">
      <table id="facTable" class="min-w-full table-auto">
        <thead class="bg-slate-50 sticky top-0 z-10">
          <tr>
            <th class="px-5 py-3 text-left text-[11px] font-semibold tracking-wider text-slate-600 uppercase">Nama Fakultas</th>
            ...
            <tr class="hover:bg-slate-50/60 transition">
              ...
              <td class="px-5 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="font-semibold text-slate-800">{{ $faculty->name }}</p>
                    <p class="text-xs text-slate-500">ID: {{ $faculty->id }}</p>
                  </div>
                </div>
              </td>
              ...
              <td class="px-5 py-4">
                <span class="inline-flex items-center gap-2 rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">
                  <i class="fa-solid fa-link text-slate-500"></i>{{ $faculty->slug }}
                </span>
              </td>
            ...
            <tr>
              <td colspan="4" class="py-16">
                <div class="mx-auto w-full max-w-md rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center">
                  ...
                  <p class="font-semibold text-slate-700">Belum ada data fakultas</p>
...
<script>
  // Filter client-side tanpa mengubah backend
  function filterTable(){
    const q = (document.getElementById('tableSearch').value || '').toLowerCase();
    const rows = document.querySelectorAll('#facTable tbody tr');
    rows.forEach(row=>{
