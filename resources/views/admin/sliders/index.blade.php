                        ...
                        <tr class="hover:bg-slate-50/60">
                            ...
                            <td class="px-5 py-4">
                                <div class="w-28 h-16 rounded-lg overflow-hidden bg-slate-100 flex items-center justify-center shadow-sm">
                                    ...
                                    {{-- ===== INI PERBAIKANNYA ===== --}}
                                    {{-- 1. Cek relasi 'images' (plural), bukan properti 'image' --}}
                                    {{-- 2. Ambil 'first()' (gambar pertama) dari relasi itu --}}
                                    {{-- 3. Ambil 'image_path' (sesuai form Anda), bukan 'image' --}}

                                    @if($slide->images && $slide->images->isNotEmpty())
                                        <img src="{{ Storage::url($slide->images->first()->image_path) }}" alt="{{ $slide->title }}" class="w-full h-full object-cover">
                                    @else
