...
<div class="space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
      <h1 class="text-3xl font-bold text-slate-800">Galeri Album</h1>
      <p class="text-sm text-slate-500">Kelola album dan foto untuk galeri kampus.</p>
    </div>

    <div class="flex items-center gap-3">
      <div class="relative">
        <input id="albumSearch" type="text" placeholder="Cari judul album…"
      ...
      <a href="{{ route('admin.albums.create') }}"
        ...
        <i class="fa-solid fa-folder-plus"></i> Buat Album
      </a>
    </div>
  </div>
  ...
  <div id="albumGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
    @forelse ($albums as $album)
      <div class="album-card group overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200 shadow-sm hover:shadow-md transition">
        <a href="{{ route('admin.albums.show', $album->id) }}" class="relative block">
          <div class="aspect-[4/3] w-full overflow-hidden bg-slate-100">
            ...
            @else
              <div class="flex h-full w-full items-center justify-center text-slate-400">
                <i class="fa-regular fa-images text-3xl"></i>
              </div>
            @endif
          </div>
          ...
          {{-- Overlay tombol lihat (hover) --}}
          <div class="pointer-events-none absolute inset-0 hidden items-center justify-center bg-black/0 transition group-hover:flex group-hover:bg-black/20">
            <span class="pointer-events-auto inline-flex items-center gap-2 rounded-xl bg-white/90 px-3 py-1.5 text-xs font-semibold text-slate-800 shadow">
        ...
        <div class="p-4">
          <h3 class="line-clamp-2 font-semibold text-slate-800">{{ $album->title }}</h3>
          <p class="mt-1 text-xs text-slate-500">ID: {{ $album->id }}</p>
    ...
    @empty
      <p class="col-span-4 text-center text-slate-500">
        Belum ada album. <a href="{{ route('admin.albums.create') }}" class="text-blue-600 hover:underline">Buat album pertama</a>.
      </p>
    @endforelse
  </div>
</div>
