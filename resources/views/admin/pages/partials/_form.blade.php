    ...
    <form action="{{ isset($page) ? route('admin.pages.update', $page->id) : route('admin.pages.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div>
                ...
                @if(isset($page) && $page->header_image)
                    <img src="{{ Storage::url($page->header_image) }}" class="w-1/2 rounded mb-2" alt="Header">
                @endif
        ...
        <div class="mb-6">
            <label class="inline-flex items-center">
                ...
                <input type="checkbox" name="active" value="1" class="form-checkbox" {{ old('active', $page->active ?? true) ? 'checked' : '' }}>
                <span class="ml-2">Aktifkan halaman ini</span>
            </label>
        </div>

        <div class="flex items-center">
            <button type="submit" class="bg-blue-500 text-white font-bold py-2 px-4 rounded">
                {{ isset($page) ? 'Perbarui' : 'Simpan' }}
            </button>
            <a href="{{ route('admin.pages.index') }}" class="ml-4">Batal</a>
        </div>
    </form>
</div>
